...
class CalcUITests: XCTestCase {
    ...
    func testDivideByZero() {
        ...
        assertCalculatorScreenReads("inf")
        
  
    }
    func assertCalculatorScreenReads(_ value: String){
        
        if let textFieldValue = app.textFields["display"].value as? String {
            XCTAssertTrue(textFieldValue == value, "Incorrect value. Was \(textFieldValue)")
        } else{
            XCTFail("Could not get this")
        }
    }
    ...
    */
    ...
    // Check that 34 characters fit, but 35 will not. Using 8's.
    func testLength() {
        var i = 0
        repeat {
        app.buttons["8"].tap()
     ...
     if let textFieldValue = app.textFields["display"].value as? String {
     XCTAssertTrue(textFieldValue == "8888888888888888888888888888888888")
        }
